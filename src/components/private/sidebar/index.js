				...
				return (
					<Fragment>
						{item.dropdowns ? (
							...
							<li className="" key={i}>
								<NavLink
									className={`nav-link text-white p-3   ${styles.sidebarlink} `}
									to={item.linkTo}
								>
									...
									>
										...
										{item.dropdowns && !this.props.changeStyle && (
											<i className="ni ni-bold-down ml-2"></i>
										)}
									</span>
								</NavLink>
							</li>
				...
				<Fragment>
					<ul className="navbar-nav mt-4">
						{!this.props.changeStyle ? (
							<h4 className="display-4 my-1 py-1 text-center text-white">
								Logo Header
							</h4>
						) : (
							""
						...
						{!this.props.changeStyle && (
							<div
								className="d-flex flex-column container my-2 py-1 text-center"
								style={{ color: "#fff" }}
							>
								<div>
